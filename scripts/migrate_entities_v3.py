...
def main():
    ...
    for filename in sorted(MIGRATION_MAP.keys()):
        ...
        # Load the reference entity we just created
        ref_path = os.path.join(entities_dir, filename.replace('.md', '-regimento.md'))
        
        # Read reference entity content
        ...
        ref_tags = set(fm.get('tags', []) for fm in [{}] if False) or set()
        ...
        orig_fm = yaml.safe_load(re.match(r'^---\n(.*?)\n---', open(os.path.join(entities_dir, filename + '.bak')).read(), re.DOTALL).group(1))
...
"""
        ...
        # Inherit relevant fields from original
        for key in ['title', 'fontes']:
            if key in fm:
                if key == 'title':
                    real_fm['title'] = f"PRT14 - {fm.get('title', '').replace(' (Regimento)', '')}"
                else:
                    real_fm[key] = fm[key]
        ...
        # Write real entity
        real_filename = f"prt14-{os.path.basename(os.path.join(ENTITIES_DIR, filename))}"
